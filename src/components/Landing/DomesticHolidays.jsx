...
const DomesticHolidays = ({ initialPackages = [], hideHeader = false }) => {
  ...
  const packages = useMemo(() => {
    // Filter for domestic packages only
    const domesticPackages = curatedPackages.filter(
      (pkg) => pkg.domestic === true
    );

    switch (filterType) {
      case "trending":
        return domesticPackages.filter((pkg) => pkg.trending);
      case "discount":
        return domesticPackages.filter((pkg) => pkg.offerPrice);
      case "bestseller":
        return domesticPackages.filter((pkg) => pkg.bestseller);
      case "curated":
      default:
...
export default DomesticHolidays;
